...
import {
  getPrintableName,
  getProfilePageLinkForIdentity,
} from "../common/model/GerritUserdata";
...
export default class OverviewTable extends React.Component {
      ...
      name: {
        ...
        cell: (record, index) => (
          <Td
            key={"name" + index}
            column="name"
            value={getPrintableName(record.identity)}
          >
            <Link
              to={getProfilePageLinkForIdentity(
                record.identifier,
  ...
  componentWillReceiveProps(nextProps) {
    Object.keys(this.state.columnMetadata).forEach(
      function (columnName) {
  ...
  onSelectAllCheckboxValueChanged() {
    ...
    const newSelectedUsers = isAllSelected
      ? selectedUsers.selectNone()
  ...
  onIdentityCheckboxValueChanged(identifier) {
    const newSelectedUsers =
      this.state.selectedUsers.toggleSelection(identifier);
    this.updateSelectedUsersForHighlighters(newSelectedUsers);
