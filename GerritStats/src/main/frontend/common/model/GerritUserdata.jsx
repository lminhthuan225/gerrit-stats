...
export function getProfilePageLinkForIdentity(
  ...
  const identifier =
    typeof identityOrIdentifier === "string"
      ? identityOrIdentifier
      : identityOrIdentifier["identifier"];

  return `/profile/${identifier}/${projectName}`;
}
...
function sortReviewDataByAddedAsReviewerCount(reviewTableData) {
  ...
  reviewerData.sort(function (l, r) {
    ...
    return lValue > rValue
      ...
      : lValue < rValue
      ? 1
      : identityCompare(l.identity, r.identity);
  });
  return reviewerData;
}
...
export default class GerritUserdata {
  constructor(userdataRecord) {
    ...
    this.datedCommitTable = new DatedList(
      this.record.commits,
      (item) => item.createdOnDate
    );
    this.datedCommentTable = new DatedList(
      allCommentsByUser,
  ...
  getReviewsGivenForScore(score, selectedUsers) {
    ...
    return this.record.reviewRequestors.reduce(
      function (prev, value) {
        const isSelected = selectedUsers.isUserSelected(value.identity);
        return (
          prev + (isSelected ? value.approvalData.approvals[scoreKey] || 0 : 0)
        );
      }.bind(this),
      0
    );
  ...
  getReviewsReceivedForScore(score, selectedUsers) {
    const scoreKey = score + "";
    return this.record.reviewersForOwnCommits.reduce(
      function (prev, value) {
        const isSelected = selectedUsers.isUserSelected(value.identity);
        return (
          prev + (isSelected ? value.approvalData.approvals[scoreKey] || 0 : 0)
  ...
  getReviewerDataForOwnCommits() {
    return sortReviewDataByAddedAsReviewerCount(
      this.record.reviewersForOwnCommits
    );
  }
  ...
  getCommitsWithHighPatchSetCount() {
    ...
    this.record.commits.forEach(function (commit) {
      ...
      var firstNonAuthorCommentPatchSetIndex =
        getFirstPatchSetIndexWithNonAuthorReview(commit);
      if (
        firstNonAuthorCommentPatchSetIndex != -1 &&
        commit.patchSets.length - firstNonAuthorCommentPatchSetIndex >
          HIGH_PATCH_SET_COUNT_THRESHOLD
      ) {
